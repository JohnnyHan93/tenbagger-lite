...
- Live URL remains https://idt.grok.me/. `JohnnyHan93/idt` is a frozen 2026-09-17 Grok export; do not develop there.
- Ported `docs/DEPLOY.md` and `docs/IDT_REFLECT.md` from the alias repo. Added `docs/REPO.md`.
- Engine scores stay unmerged. `EXECUTE_FULL_100` stays locked. GitHub push still does not publish grok.me.

## XBG-v2.3 — observed growth F10 + honest trust

- F10 stays N/A without `revenuePrior`. No 0% CAGR / fixed EV/S scenario pack. Required CAGR still shows on Tenx math.
- Trust is RESEARCH REQUIRED until management or disclosure evidence exists. Going concern still FAILs.
- TURNAROUND tag requires a complete Oversold Case.
- Canonical code stays `JohnnyHan93/tenbagger-lite`. Live URL is `idt.grok.me`. `JohnnyHan93/idt` is a frozen export.
- Past snapshots unchanged. Scores stay unmerged. `EXECUTE_FULL_100` locked.
