...
import { formatScreenFlags, xbaggerScreen, XBG_SCR_VERSION } from "@/lib/engines/xbagger-screen";
...
function Page() {
  ...
  return (
    <>
      <PageTitle kicker={`${ENGINE_TAB.xbagger.version} · ${XBG_SCR_VERSION}`} title={ENGINE_TAB.xbagger.name} />
      <p className="mb-4 max-w-2xl text-sm text-muted">
        10 Factor × 0–10, 가중합 0–100. N/A는 분모에서 제외. Hard Gate(10x / Survival)는 평균과 섞지 않습니다.
        스크리닝(런웨이·희석·PoC)은 점수에 더하지 않습니다.
      </p>
        ...
        <table className="idt-table w-full min-w-[640px] text-left text-sm">
          <thead className="border-b border-border font-mono text-[0.625rem] tracking-widest text-subtle uppercase">
            <tr>
              ...
              <th className="px-3 py-3">스크리닝</th>
          ...
          <tbody className="divide-y divide-border">
            ...
            {rows.map((r) => {
              const screen = xbaggerScreen({ xbagger: r.snapshot.xbagger, derived: r.snapshot.derived });
              return (
                <tr key={r.company.id}>
                  ...
                  <td className="px-3 py-3 text-xs text-flag-yellow" title={screen.map((f) => f.reason).join(" ")}>
                    {formatScreenFlags(screen)}
                  </td>
