...
function trustGate(
  trustFail: boolean | undefined,
  signals: TrustSignals,
): XGates["trust"] {
  if (trustFail || signals.auditGoingConcern) return "FAIL";
  if (signals.management === false || signals.disclosure === false) return "FAIL";
  if (signals.management === true || signals.disclosure === true) return "PASS";
  return "RESEARCH REQUIRED";
}
...
export function scoreXBagger(input: XBaggerInput, criteria?: XBaggerCriteria): XBaggerResult {
  ...
  const mathOk = Boolean(
    input.tenxMath &&
      input.tenxMath.currentRevenue != null &&
      input.tenxMath.assumedCagr != null &&
      input.tenxScenarios.length > 0,
  );
  ...
  const gates: XGates = {
    trust: trustGate(input.trustFail, trustSignals),
